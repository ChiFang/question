...
CUDA_FANG_INCLUDE = /usr/local/cuda-9.0/targets/x86_64-linux/include

CC        = gcc -O2 -pthread
CXX       = g++
GPUCC     = nvcc
# CFLAGS    = -std=c++11 -I$(TF_INC) -I"$(CUDA_HOME)/include" -DGOOGLE_CUDA=1
CFLAGS    = -std=c++11 -I$(TF_INC) -I"$(CUDA_HOME)/include" -I/usr -I"$(CUDA_HOME)" -I"/usr/local/lib/python3.5/dist-packages/tensorflow/include/external/nsync/public"
GPUCFLAGS = -c
...
clean:
	rm -f $(PREPROCESSING_PROD) $(GPU_PROD_FLOW) $(GPU_PROD_DATA_AUG) $(DOWNSAMPLE_PROD) $(GPU_PROD_DOWNSAMPLE) $(FLOWWARP_PROD) $(GPU_PROD_FLOWWARP) $(CORRELATION_PROD) $(GPU_PROD_CORRELATION)
